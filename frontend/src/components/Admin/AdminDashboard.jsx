...
// User Profile Popup Component - FIXED
const UserProfilePopup = ({ userId, isOpen, onClose }) => {
  const [userProfile, setUserProfile] = useState(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  ...
  return (
    <>
      ...
      {/* Modal - centered with proper z-index */}
...
const AdminDashboard = () => {
  ...
  // FIXED: Better state management for popup
  const [showUserProfile, setShowUserProfile] = useState(false);
  const [selectedUserId, setSelectedUserId] = useState(null);
  ...
  // FIXED: Handler for viewing user profile
  const handleViewProfile = (userId) => {
    setSelectedUserId(userId);
    setShowUserProfile(true);
  };

  // FIXED: Close handler with proper cleanup
  const handleCloseUserProfile = () => {
    setShowUserProfile(false);
    setSelectedUserId(null);
  };

  // Click outside handler for the main content
  const handleMainContentClick = (e) => {
    // Only close if clicking on the backdrop and not the popup content
    if (e.target === e.currentTarget) {
      handleCloseUserProfile();
    }
  };

  // Rest of your existing functions (debugAuth, getAuthHeaders, fetchAllUsers, etc.)
  const debugAuth = () => {
    const user = JSON.parse(localStorage.getItem("user") || "{}");
    const token = localStorage.getItem("access");
      ...
      <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-4 mb-4">
          ...
          <div className="flex items-center flex-1 min-w-0">
            ...
            <div className="ml-3 flex-1 min-w-0">
              <button
                ...
                className="text-base font-semibold text-gray-900 hover:text-blue-600 hover:underline text-left truncate block w-full"
              >
                {user.username || 'Unknown User'}
              </button>
        ...
        <div className="flex items-center text-sm text-gray-600 mb-3">
          ...
          <span className="truncate">
            Registered: {user.date_joined ? new Date(user.date_joined).toLocaleDateString() : 'Invalid Date'}
          </span>
        </div>
    ...
    return (
      <tr className="hover:bg-gray-50 transition-colors">
        <td className="px-4 py-3 whitespace-nowrap">
          <div className="flex items-center">
            <div 
              className="flex-shrink-0 h-8 w-8 sm:h-10 sm:w-10 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center cursor-pointer hover:from-blue-600 hover:to-purple-700 transition-all"
              onClick={() => handleViewProfile(user.id)}
            >
            ...
            <div className="ml-3">
              <button
                onClick={() => handleViewProfile(user.id)}
                className="text-xs sm:text-sm font-semibold text-gray-900 hover:text-blue-600 hover:underline text-left"
              >
                {user.username || 'Unknown User'}
              </button>
          ...
          <div className="flex items-center space-x-2">
            ...
            {isInPendingList ? (
              <>
                <button
                  onClick={() => handleApprove(user.id)}
                  className="bg-green-600 text-white px-2 sm:px-3 py-1 rounded-md hover:bg-green-700 transition-colors text-xs"
                >
                  Approve
                </button>
                <button
                  onClick={() => handleReject(user.id)}
                  className="bg-red-600 text-white px-2 sm:px-3 py-1 rounded-md hover:bg-red-700 transition-colors text-xs"
                >
                  Reject
                </button>
              </>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-3 sm:p-4 lg:p-8 mt-16 sm:mt-20">
      {/* User Profile Popup */}
      <UserProfilePopup 
        userId={selectedUserId}
        isOpen={showUserProfile}
      ...
      <div className="max-w-7xl mx-auto">
        <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-center mb-4 sm:mb-6 lg:mb-8 text-gray-900">
          User Management Dashboard
        </h1>
        ...
        <button 
          onClick={debugAuth}
          className="fixed top-4 right-4 bg-gray-800 text-white px-2 py-1 rounded text-xs opacity-50 hover:opacity-100 z-10"
          title="Debug Auth"
        >
          🔍 Debug
        </button>
        ...
        {/* Tabs */}
        <div className="bg-white rounded-xl shadow-sm mb-4 sm:mb-6 border border-gray-200 overflow-hidden">
          <div className="flex flex-wrap sm:flex-nowrap">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id)}
                className={`flex-1 min-w-[33.333%] sm:min-w-0 py-3 sm:py-4 px-2 sm:px-6 text-center border-b-2 font-medium text-xs sm:text-sm transition-colors ${
                  activeTab === tab.id
                    ? "border-blue-500 text-blue-600 bg-blue-50"
                    : "border-transparent text-gray-500 hover:text-gray-700 hover:bg-gray-50"
                }`}
