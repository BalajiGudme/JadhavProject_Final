...
const ArchitectureManager = () => {
  ...
  // State to track if batch endpoint is available
  ...
  // Function to test if batch endpoint is available
  const testBatchEndpoint = async () => {
    try {
      const response = await authFetch(`${API_BASE_URL}/architectures/batch-responses/`, {
        method: 'OPTIONS' // Simple preflight request to check if endpoint exists
      });
      setUseBatchEndpoint(response.status !== 404);
    } catch (error) {
      console.log('Batch endpoint not available, using individual requests');
      setUseBatchEndpoint(false);
    }
  };
  ...
  const fetchArchitectures = async () => {
    try {
      ...
      // Fetch token counts for each architecture using batch or individual requests
      await fetchTokenCounts(processedArchitectures);
  ...
  // Fetch token counts for each architecture (optimized with batch)
  const fetchTokenCounts = async (archs) => {
    const counts = {};
    
    if (archs.length === 0) return counts;
    
    if (useBatchEndpoint) {
      // Use batch endpoint for efficiency
      const architectureIds = archs.map(arch => arch.id);
        ...
        if (response.ok) {
          ...
          if (data.architectures && Array.isArray(data.architectures)) {
            data.architectures.forEach(arch => {
              // Extract unique tokens from responses
              const tokens = arch.responses 
                ? [...new Set(arch.responses
                    .map(response => response.token)
                    .filter(Boolean))]
                : [];
              counts[arch.architecture_id] = tokens.length;
            });
          }
        } else {
          // Fallback to empty counts if batch request fails
          architectureIds.forEach(id => { counts[id] = 0; });
          console.error('Batch request failed:', response.status);
        }
      } catch (error) {
        console.error('Error fetching batch responses:', error);
        // Fallback to individual requests if batch fails
        await fallbackFetchTokenCounts(archs, counts);
      }
    } else {
      // Use individual requests (fallback)
      await fallbackFetchTokenCounts(archs, counts);
    }
  ...
  // Fallback function for individual token count requests
  const fallbackFetchTokenCounts = async (archs, counts) => {
    for (const arch of archs) {
      try {
        const response = await authFetch(`${API_BASE_URL}/architecture/${arch.id}/responses/`);
        if (response.ok) {
          const data = await response.json();
          const tokens = data.responses 
  ...
  // Fetch all responses for architectures using batch endpoint (optimized)
  const fetchArchitectureResponses = async (architecturesList) => {
    if (!architecturesList || architecturesList.length === 0) {
      return [];
    }
    ...
    if (useBatchEndpoint) {
      // Use batch endpoint for efficiency
      const architectureIds = architecturesList.map(arch => arch.id);
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.architectures && Array.isArray(data.architectures)) {
            // Find the corresponding architecture for each response
            data.architectures.forEach(arch => {
              const originalArch = architecturesList.find(a => a.id === arch.architecture_id);
              
              if (arch.responses && Array.isArray(arch.responses) && arch.responses.length > 0) {
                const responsesWithArchInfo = arch.responses.map(resp => ({
                  ...resp,
                  architecture_id: arch.architecture_id,
                  architecture_name: originalArch?.name || arch.architecture_name,
                  architecture_class: originalArch?.class_name || '',
                  architecture_department: originalArch?.department_name || '',
                  architecture_institution: originalArch?.institution_type || '',
                  architecture_division: originalArch?.division || '',
                  architecture_student_count: originalArch?.student_count || 0,
                  architecture_staff_count: originalArch?.staff_count || 0
                }));
          ...
          return await fallbackFetchArchitectureResponses(architecturesList);
        }
      } catch (error) {
  ...
  // Fallback function for individual architecture responses requests
  const fallbackFetchArchitectureResponses = async (architecturesList) => {
    const allResponses = [];
